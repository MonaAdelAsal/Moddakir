package com.moddakir.mylibrary.view.agora;
...
import com.moddakir.mylibrary.Model.BaseResponse;
import com.moddakir.mylibrary.SinchEx.AudioPlayer;
import com.moddakir.mylibrary.SinchEx.MainCallScreen;
import com.moddakir.mylibrary.view.widget.ButtonCalibriBold;
import com.moddakir.mylibrary.view.widget.TextViewCalibriBold;
import com.moddakir.mylibrary.view.widget.TextViewLateefRegOT;
import com.moddakir.mylibrary.view.widget.TextViewUniqueLight;
import com.moddakir.mylibrary.utils.CallStatus;
import com.moddakir.mylibrary.utils.LogFile;
import com.moddakir.mylibrary.utils.SharedPrefUtil;
import com.moddakir.mylibrary.utils.Utils;
import com.moddakir.mylibrary.API.ApiManager;
import com.moddakir.mylibrary.Adapters.ConnectingScreenAdapter;
import com.moddakir.mylibrary.Adapters.DotsIndicatorDecoration;
import com.moddakir.mylibrary.Model.CallLog;
import com.moddakir.mylibrary.Model.ConnectingBannersResponse;
import com.moddakir.mylibrary.Model.ConsumedPakageResponseModel;
import com.moddakir.mylibrary.Model.CreateCallResponseModel;
import com.moddakir.mylibrary.Model.FreezeAccountResponseModel;
import com.moddakir.mylibrary.Model.LoginResponseModel;
import com.moddakir.mylibrary.Model.Plan.afterCallResponse;
import com.moddakir.mylibrary.Model.RandomTeacherResponseModel;
import com.moddakir.mylibrary.Model.ResponseModel;
import com.moddakir.mylibrary.Model.User;
import com.moddakir.mylibrary.R;
import com.moddakir.mylibrary.utils.AccountPreference;
import com.moddakir.mylibrary.utils.Perference;
import com.moddakir.mylibrary.view.evaluation.TeacherEvaluationActivity;
...
import static com.moddakir.mylibrary.Constants.CALL_IDS;
                    ...
                    public void onSuccess(Response<CreateCallResponseModel> response) {
                        if (response.isSuccessful() && response.body() != null) {
                            ...
                            if (response.body().getStatusCode() == 417) {
                                ...
                                // setupVideoSDKEngine(response.body().getCallApiKey());
...
//                        alertDialog.dismiss();
    ...
    private void startCall(CreateCallResponseModel createCallResponse) {
        ...
        if (createCallResponse.getEnableCallLogs()) {
            ...
            ArrayList<String> listOfCallIds = SharedPrefUtil.getSharedPref(getApplicationContext()).read(CALL_IDS);
            if (listOfCallIds == null || listOfCallIds.size() <= 0) {
                listOfCallIds = new ArrayList<>();
            }
            listOfCallIds.add(createCallResponse.getCall().getCid());
            SharedPrefUtil.getSharedPref(getApplicationContext()).write(CALL_IDS, listOfCallIds);
        }
    ...
    private void showUnFreezeAccountDialog() {
        SweetAlertDialog sAlertDialog = new SweetAlertDialog(this, SweetAlertDialog.NORMAL_TYPE);
        sAlertDialog.setTitleText(getResources().getString(R.string.unfreeze_account_message));
        sAlertDialog.setCancelable(true);
        sAlertDialog.setConfirmText(getResources().getString(R.string.ok));
        sAlertDialog.setCancelText(getResources().getString(R.string.cancel));
        sAlertDialog.setConfirmClickListener(sweetAlertDialog -> {
            sAlertDialog.dismissWithAnimation();
            unFreezeAccount();
        });
        sAlertDialog.setCancelClickListener(sweetAlertDialog -> {
            sAlertDialog.dismissWithAnimation();
            playEndCallSound();
            backToHomePage();
        ...
        sAlertDialog.show();
        Utils.changeSweetAlertDialogStyle(sAlertDialog, this);

    }

    private void unFreezeAccount() {
        AlertDialog dialog = Perference.ShowProgress(this);
        dialog.show();
        (new ApiManager(AgoraActivity.this).getUserCalls(true)).unFreezeAccount()
                .subscribeOn(Schedulers.io()) // “work” on io thread
                .observeOn(AndroidSchedulers.mainThread()) // “listen” on UIThread
                .map(responseModel -> responseModel)
                .subscribe(new DisposableSingleObserver<FreezeAccountResponseModel>() {
                    @Override
                    public void onSuccess(@NonNull FreezeAccountResponseModel freezeAccountResponseModel) {
                        if (freezeAccountResponseModel.getStatusCode() == 200) {
                            playEndCallSound();
                            Toast.makeText(AgoraActivity.this, getResources().getString(R.string.account_unfrozen_sucessfully), Toast.LENGTH_LONG).show();
                        }
                        dialog.dismiss();
                        finish();
                    }
                    ...
                    public void onError(@NonNull Throwable e) {
                        Toast.makeText(AgoraActivity.this, getResources().getString(R.string.somethingWrong), Toast.LENGTH_LONG).show();
                        dialog.dismiss();
                        finish();
                    }
                });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Toast.makeText(this,"Agora",Toast.LENGTH_LONG).show();
        ...
        disposable = Observable.timer(3, TimeUnit.SECONDS).repeat().subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()).
                subscribe(aLong -> {
                    ...
                    if (max < 1.8 && max >= 0) {
                        ...
                        //good
                        tv_internet_status.setText(com.moddakir.mylibrary.R.string.internet_speed_good);
                        ...
                        //weak
                        tv_internet_status.setText(com.moddakir.mylibrary.R.string.internet_speed_weak);
                        tv_internet_status.setBackgroundResource(R.color.colorGray);

                        tv_internet_status.setVisibility(View.VISIBLE);
                    } else {
                        //bad
                        tv_internet_status.setText(com.moddakir.mylibrary.R.string.internet_speed_bad);
    ...
    private void InitViews() {
        loginSDK("male", "Ahmed", "+201112072662");
        mAudioPlayer = new AudioPlayer(this);
    ...
    protected void StatusChanged(int integer) {
        if (integer == 1) {
            speed_status.setText(com.moddakir.mylibrary.R.string.internet_speed_low);
            tv_internet_status.setText(com.moddakir.mylibrary.R.string.internet_speed_low);
            tv_internet_status.setBackgroundResource(R.color.colorGray);
            speed_status.setBackgroundResource(R.color.colorGray);
        } else if (integer == 2) {
            speed_status.setText(com.moddakir.mylibrary.R.string.internet_speed_good);
            tv_internet_status.setText(com.moddakir.mylibrary.R.string.internet_speed_good);
            tv_internet_status.setBackgroundResource(R.color.colorPrimary);
            speed_status.setBackgroundResource(R.color.colorPrimary);
        } else if (integer == 0) {
            speed_status.setText(com.moddakir.mylibrary.R.string.no_internet_speed);
            tv_internet_status.setText(com.moddakir.mylibrary.R.string.no_internet_speed);
            tv_internet_status.setBackgroundResource(R.color.colorDarkGray);
    ...
    private void CallUserSinch() {
        ...
        if (ex != null) {
            teacher = new Gson().fromJson(ex.getString("teacher"), User.class);
            if (teacher != null) {
                mCallerName1.setText(teacher.GetCalleeName());
                Utils.loadAvatar(this, teacher.getAvatarurl(), civ_teacher_image);
                Utils.loadAvatar(this, teacher.getAvatarurl(), civ_teacher_image1);
            ...
            public void onMessageReceived(RtmMessage message, RtmChannelMember fromMember) {
                ...
                if (text.equals("error")) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        warning_frame.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
    ...
    private void loginSDK(String gender, String name, String phone) {
        HashMap<String, Object> map = new HashMap<>();
                ...
                .subscribe(new DisposableSingleObserver<Response<LoginResponseModel>>(){
                    ...
                    public void onSuccess(Response<LoginResponseModel> response) {
                        if (response.isSuccessful() && response.body() != null) {
                            Timber.e(new Gson().toJson(response.body()));
                            if (response.body().getStatusCode() == 200) {
                                User user = response.body().getStudent();
                                user.setAccessToken(response.body().getAccessToken());
                                AccountPreference.registerData(user,AgoraActivity.this);
                                getRandomTeacher();
    ...
    public static void CallRandomTeacher(Context context, String gender, String name, String phone) {
